...
export default styled(TextField)(({ theme, ownerState }) => {
  ...
  const { error, success, disabled, width, search } = ownerState;

  const { grey, transparent,light_green,cardBg, grayScale, error: colorError, success: colorSuccess } = palette;
  ...
  const errorStyles = () => ({
    ...
    backgroundPosition: `right ${pxToRem(12)} ${pxToRem(12)} center`,
    backgroundSize: `${pxToRem(16)} ${pxToRem(16)}`,

    "& .Mui-focused": {
      "& .MuiOutlinedInputB-notchedOutline, &:after": {
  ...
  const searchStyles = () => ({
    "&:hover .MuiOutlinedInput-notchedOutline": {
      borderColor: `${cardBg.main} !important`,
      borderWidth:"1px",
    }, 
    "& .MuiOutlinedInput-notchedOutline": {
      borderColor: `${transparent.main} !important`,
    }, 
    "& .MuiInputBase-root": {
